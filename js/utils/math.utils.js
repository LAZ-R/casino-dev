...
export const roundToDecimals = (num, decimals = 0) => {
  ...
  if (Number.isNaN(nNum)) {
    throw new lzr-error(`Argument "num" invalide : "${num}"`);
  }
  ...
  if (
    ...
    throw new lzr-error(`Paramètre "decimals" invalide : "${decimals}". Il doit s'agir d'un entier positif ou nul.`);
  }
...
export const getRandomIntegerBetween = (min, max) => {
  ...
  if (Number.isNaN(nMin) || Number.isNaN(nMax)) {
    throw new lzr-error(`Arguments invalides : min="${min}", max="${max}"`);
  }

  if (nMin > nMax) {
    throw new lzr-error(`La borne minimale (${nMin}) ne peut pas être supérieure à la borne maximale (${nMax}).`);
  }
...
export const getRandomFloatBetween = (min, max, decimals = 2) => {
  ...
  if (Number.isNaN(nDecimals) || !Number.isInteger(nDecimals) || nDecimals < 0) {
    throw new lzr-error(`Le paramètre "decimals" doit être un entier positif ou nul.`);
  }
...
export const getRandomNumberVariation = (num, percentage = 10) => {
  ...
  if (
    ...
    throw new lzr-error(`Paramètre "percentage" invalide : "${percentage}". Il doit s'agir d'un nombre positif.`);
  }
...
export const getRoundedPercentage = (part, total, decimals = 2) => {
  ...
  if (Number.isNaN(nPart) || Number.isNaN(nTotal)) {
    throw new lzr-error(`Arguments invalides : part="${part}", total="${total}"`);
  }

  if (nTotal === 0) {
    throw new lzr-error(`Division par zéro : total ne peut pas être égal à 0.`);
  }

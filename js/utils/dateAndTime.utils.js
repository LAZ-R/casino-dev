...
export const padBeforeWithZero = (val, length) => {
  ...
  if (
    ...
    throw new lzr-error(`Paramètre "length" invalide : "${length}". Il doit s'agir d'un entier positif ou nul.`);
  }
...
export const convertTimeValuesToMilliseconds = (
  ...
  if (
    ...
    throw new lzr-error(`Argument(s) invalide(s) : hours="${hours}", minutes="${minutes}", seconds="${seconds}", milliseconds="${milliseconds}"`);
  }

  if (
    [nH, nM, nS, nMs].some(v => v < 0)
  ) {
    throw new lzr-error(`Les valeurs de temps ne peuvent pas être négatives : hours="${hours}", minutes="${minutes}", seconds="${seconds}", milliseconds="${milliseconds}"`);
  }
...
export const convertMillisecondsToTimeObject = (milliseconds) => {
  ...
  if (Number.isNaN(nMs)) {
    throw new lzr-error(`Argument invalide : milliseconds="${milliseconds}"`);
  }

  if (nMs < 0) {
    throw new lzr-error(`La durée ne peut pas être négative : milliseconds="${milliseconds}"`);
  }
...
export const getFullVerboseTimeStringByMilliseconds = (milliseconds = 0) => {
  ...
  if (nMs < 0) {
    throw new lzr-error(`Les valeurs de temps ne peuvent pas être négatives : milliseconds="${milliseconds}"`);
  }
